...
--    SZ, SS, GEM (创业板), STAR (科创板), BJ, HK, OVERSEAS (non-Greater-China)
-- ============================================================================
...
COMMENT ON COLUMN stats.sec_classification.exchange              IS 'Listing board/market: SZ, SS, GEM (创业板), STAR (科创板), BJ, HK (港股通/港交所), OVERSEAS (non-Greater-China QDII targets — US/Japan/Europe/emerging). NULL for synthetic DUMMY indices. Indices use bare 6-digit codes with exchange derived from code prefix (000/930/931/932/950/990→SS, 399→SZ, 899→BJ, H/CES→HK) + name/sector overrides (HK-themed names→HK, OVERSEAS sector→OVERSEAS). ETFs inherit from CSV 上市地 or code suffix, overridden to HK/OVERSEAS when the underlying market is HK/non-Greater-China (name keywords: 标普/纳斯达克/美国/日经/德国/法国/欧洲/巴西/沙特/印度/越南/亚太精选 → OVERSEAS; 港股通/恒生/沪港深/SHS → HK). Stocks use code suffix only (.SZ/.SS/.BJ) — A-shares held by HK/overseas-themed indices keep their own exchange.';
COMMENT ON COLUMN stats.sec_classification.is_primary_exchange  IS 'Derived flag — ALWAYS generated by the rule (post-upsert UPDATE in build_classification sector_industry/upsert.py, same pattern as is_active): TRUE for Greater-China primary exchanges (SS, STAR, SZ, GEM, BJ); FALSE for cross-border / non-Greater-China (HK, OVERSEAS); NULL when exchange IS NULL (synthetic DUMMY indices). Splits the UI exchange filter into two rows: primary (SSE/SZSE/BSE) and cross-border (HK/Overseas). Never set manually — re-derived from `exchange` every build.';
...
-- NOTE: stats.sec_owners has been MOVED to 11_sec_info.sql and is now populated
-- by `python -m builds.sec_info` (not build_classification.py).  The logical
-- owner_id reference on sec_classification is still matched in-memory by
-- builds.classification via builds.classification.sector_industry.owners
-- (load_owners + build_owner_matchers + match_etf_owner) — no DB sec_owners
-- table is needed at classification time.
