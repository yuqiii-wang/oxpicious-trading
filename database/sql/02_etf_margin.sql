...
--  Split into: etf_identity, etf_ohlcv, etf_adjustment, etf_liquidity_margin, etf_composition_link
...
-- ----------------------------------------------------------------------------
-- Table: etf_composition_link
--   ← Composition snapshot linkage (merge_asof match)
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS stats.etf_composition_link (
    date                      DATE          NOT NULL,
    code                      TEXT          NOT NULL,
    comp_match_date           DATE,

    CONSTRAINT pk_etf_composition_link PRIMARY KEY (date, code),
    CONSTRAINT fk_etf_composition_link_date_code FOREIGN KEY (date, code) REFERENCES stats.etf_identity(date, code)
);

COMMENT ON TABLE  stats.etf_composition_link         IS 'ETF composition snapshot linkage (merge_asof match).';
