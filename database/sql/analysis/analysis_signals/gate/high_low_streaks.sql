-- ============================================================================
--  analysis_signals confirmation gate — family high_low_streaks
--
...
--  config filter = none (every band combo)
--
...
WITH bp AS (
    ...
    SELECT i.stat_month, m.band_period::text || '_' || pct_type::text AS win, m.side, i.code, i.sec_type,
    ...
    WHERE i.sec_type = $1 AND i.bucket = 'high_low_streaks'
      -- (every band combo — no slice filter)
