...
--  window ENDING at the month's last trading row (60/120/255/500/750/
--  1275 trading rows = ~0.25/0.5/1/2/3/5 trading years — the ma255
--  yearly-window precedent). pct_type 1 -> near-full range of the
--  window ([1st pct of lows, 99th pct of highs]); pct_type 10 -> core
--  envelope ([10th, 90th]). One band per calendar month per
--  (sec_type, code) per (period, pct_type) — 18 bands per month.
--
...
--  when all 18 (period, pct_type) rows exist; --force rebuilds the
--  whole scope.
-- ----------------------------------------------------------------------------
...
COMMENT ON TABLE  analysis.mov_ave_high_low_pct            IS 'Multi-period high/low price percentile BAND (ETF + Index + Stock). One row per (sec_type, code, date_year_month, period, pct_type): a symmetric quantile envelope of the security''s daily price range over the lookback period — low_val = the pct_type-th percentile of daily LOW prices, high_val = the (100 - pct_type)-th percentile of daily HIGH prices (linear interpolation), both over the TRAILING (backward-only) window of `period` trading rows (60/120/255/500/750/1275 = ~0.25/0.5/1/2/3/5 trading years — the ma255 yearly-window precedent) ENDING at date_year_month''s last trading row. pct_type 1 = near-full range of the window ([1st pct of lows, 99th pct of highs]); pct_type 10 = core envelope ([10th, 90th]). One band per calendar month, anchored at the month''s last trading row and stored under the month''s first day — 18 bands (6 periods x 3 pct_types) per (sec_type, code, month). Windows near a code''s history start are naturally truncated (a period needs only its own window length of observations); pair detection expects a month only once 255 rows (1 trading year) of history exist. Trailing windows make historical bands immutable once computed — only missing (code, month) pairs are computed incrementally (a pair is complete when all 18 rows exist); --force rebuilds the whole scope. Internal step of analyze.mov_ave_spread (high_low_pct.py), reusing the parent source DataFrame''s high/low columns.';
...
COMMENT ON COLUMN analysis.mov_ave_high_low_pct.period     IS 'Lookback window length in trading rows: 60 / 120 / 255 / 500 / 750 / 1275 (~0.25 / 0.5 / 1 / 2 / 3 / 5 trading years — the ma255 yearly-window precedent). The window is TRAILING — it ends inclusive at date_year_month''s last trading row and reaches back `period` rows. Part of the PK (one band set per period); near a code''s history start the window is naturally truncated to the available rows.';
