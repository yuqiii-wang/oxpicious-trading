...
--  stat_date live ONLY in analysis_forecasts.forecast_identities (one
...
--
--  Full-window gate: a code enters a stat_date only once its OWN
...
--  earlier stat_dates have no rows for it (no partial-window stats).
--
...
--                  rsi_window / ma_window keys (sec_type / stat_date
...
--  Comments — mov_rsi
-- ----------------------------------------------------------------------------
COMMENT ON TABLE analysis_forecasts.mov_rsi IS 'RSI extreme-day bucket definitions (motivation): one row per forecast_id — the window days of one security snapshot whose rsi_{W}days is in the top/bottom pct% of the trailing 10-year window ending at the bucket''s stat_date, streak-merged (2026-09: consecutive bucket days are ONE forecast signal anchored at the run''s MID day — the mean run length per signal lives on forecast_identities.streak_signal_days; the legacy fixed-5-day cooldown was removed), split by the market regime (calm/hot/panic/quiet — stats.market_regimes) its trigger days carry. PK (code, forecast_id) on HASH (code) partitions — code-clustered reads/writes; the identity (sec_type, stat_date) + bucket family live in analysis_forecasts.forecast_identities. Results (forward changes / reversal probabilities) live in analysis_forecasts.forecast_results via forecast_id. Bucket day RSI values join from analysis.mov_ave_rsi on (sec_type, code, date, rsi_window). Source: analysis.mov_ave_rsi + stats.*_basic_stats closes.';
COMMENT ON COLUMN analysis_forecasts.mov_rsi.code IS 'Hash partition key + PK lead. The bucket''s sec_type and stat_date are registered in analysis_forecasts.forecast_identities under the row''s forecast_id.';
COMMENT ON COLUMN analysis_forecasts.mov_rsi.forecast_id IS 'Surrogate id (PK partner of code; 1:N link to the bucket''s 4 period rows in analysis_forecasts.forecast_results, allocated by the writer, shared across all 4 periods). The bucket''s identity (sec_type, stat_date) + bucket family are registered in analysis_forecasts.forecast_identities under this id; id-only lookups use idx_mov_rsi_forecast_id.';
...
COMMENT ON COLUMN analysis_forecasts.mov_rsi.lookback_period IS 'Recorded build parameter (NOT a PK member): the trailing calendar window the bucket was computed over — ''10y'' = (stat_date - 10 years, stat_date]. Default ''10y''; a rebuild with a different lookback requires --force.';

-- ----------------------------------------------------------------------------
--  Comments — mov_std
-- ----------------------------------------------------------------------------
COMMENT ON TABLE analysis_forecasts.mov_std IS 'Bollinger-breach bucket definitions (motivation): one row per forecast_id — the window days of one security snapshot within the trailing 10-year window ending at the bucket''s stat_date whose price closed beyond ma_{W} ± k·std_{W}days, streak-merged (2026-09: consecutive breach days are ONE forecast signal anchored at the run''s MID day — the mean run length per signal lives on forecast_identities.streak_signal_days; the legacy fixed-5-day cooldown was removed). PK (code, forecast_id) on HASH (code) partitions — code-clustered reads/writes; the identity (sec_type, stat_date) + bucket family live in analysis_forecasts.forecast_identities. Results (forward changes / reversal probabilities) live in forecast_results linked via forecast_id. Band inputs join from analysis.mov_ave_spreads_detail / stats.*_tech_stats. Sources: stats.*_tech_stats (ma), analysis.mov_ave_spreads_detail (std), stats.*_basic_stats closes (price, COALESCE etf_adjustment.adj_close for ETFs).';
COMMENT ON COLUMN analysis_forecasts.mov_std.code IS 'Hash partition key + PK lead. The bucket''s sec_type and stat_date are registered in analysis_forecasts.forecast_identities under the row''s forecast_id.';
COMMENT ON COLUMN analysis_forecasts.mov_std.forecast_id IS 'Surrogate id (PK partner of code; 1:N link to the bucket''s 4 period rows in analysis_forecasts.forecast_results, allocated by the writer, shared across all 4 periods). The bucket''s identity (sec_type, stat_date) + bucket family are registered in analysis_forecasts.forecast_identities under this id; id-only lookups use idx_mov_std_forecast_id.';
...
COMMENT ON COLUMN analysis_forecasts.mov_std.lookback_period IS 'Recorded build parameter (NOT a PK member): the trailing calendar window the bucket was computed over — ''10y'' = (stat_date - 10 years, stat_date]. Default ''10y''; a rebuild with a different lookback requires --force.';
