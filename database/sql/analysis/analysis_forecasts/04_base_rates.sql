...
--      reverse_threshold — the SAME (fixed 1%) reversal bar the bucket
...
CREATE TABLE IF NOT EXISTS analysis_forecasts.base_rates (
    ...
    reverse_threshold NUMERIC(8,6) NOT NULL DEFAULT 0.01,  -- the (fixed 1%) reversal bar the probs (and the bucket rows) use
...
--  Comments
-- ----------------------------------------------------------------------------
COMMENT ON TABLE analysis_forecasts.base_rates IS 'Unconditional same-window base rates for the forecast analysis: per (sec_type, code, stat_month, period) the mean n-day forward fractional change, P(change < −reverse_threshold) and P(change > +reverse_threshold) over ALL of the code''s trading days in the trailing 5-year window ending at stat_month (not just the extreme bucket days), plus the valid-day count and the SAME FIXED 1% reverse_threshold the bucket rows use (lift stays in one scale). Reference for reading forecast_results.ave_change / reverse_prob as lift. Same window / price space / full-window gate as the mov_* tables. Populated by python -m analyze.analysis_forecasts.';
