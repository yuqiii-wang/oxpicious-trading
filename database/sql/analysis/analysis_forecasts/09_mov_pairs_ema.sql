...
--  MA-pair cross family on analysis.mov_ave_spreads_detail.ma5_vs_ma{W}).
--  Identical bucket machinery, built instead on the EXISTING relative
--  EMA-spread columns of analysis.mov_ave_spreads_detail_ema —
--  ema6_vs_ema{W} = (ema6 - ema_{W}) / ema_{W}, the parent mov_ave_spread
--  analysis's own EMA spread definition (no new EMA computation).
--
--  A day joins a bucket when the pair's spread changes sign that day:
--
--    side=top    — CROSS UP   (golden cross): ema6_vs_ema{W}[t] >  0 and
--                  ema6_vs_ema{W}[t-1] <= 0 (the fast EMA6 rises through
--                  the slow EMA_{W} — the pair turns bullish)
--    side=bottom — CROSS DOWN (death  cross): ema6_vs_ema{W}[t] <  0 and
--                  ema6_vs_ema{W}[t-1] >= 0 (the fast EMA6 falls through
--                  the slow EMA_{W} — the pair turns bearish)
--
--  (the [t-1] leg is the code's PREVIOUS trading-grid row; NULL
--  spreads — either EMA still warming up — never trigger). Triggers are
...
--  (the slow leg; ema6_vs_ema20 exists in the source but is not built).
--
...
--  idx_mov_pairs_ema_forecast_id). code is the ONLY identity column stored
...
BEGIN
    ...
    WHERE n.nspname = 'analysis_forecasts' AND c.relname = 'mov_pairs_ema';
    SELECT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_schema = 'analysis_forecasts'
                     AND table_name   = 'mov_pairs_ema'
    ...
    IF v_partkey IS NULL
       OR (v_partkey = 'HASH (code)'
        ...
        RETURN;
    END IF;
    ALTER TABLE analysis_forecasts.mov_pairs_ema RENAME TO mov_pairs_ema_pk_rebuild;
    ALTER TABLE analysis_forecasts.mov_pairs_ema_pk_rebuild
    ...
    PERFORM public.create_hash_partitions('analysis_forecasts',
                                          'mov_pairs_ema_new', 16);
    ALTER TABLE analysis_forecasts.mov_pairs_ema_pk_rebuild
        ADD COLUMN IF NOT EXISTS lookback_period TEXT NOT NULL DEFAULT '5y';
    IF v_has_code THEN
        INSERT INTO analysis_forecasts.mov_pairs_ema_new
        ...
        FROM    analysis_forecasts.mov_pairs_ema_pk_rebuild;
    ELSE
        ...
        FROM    analysis_forecasts.mov_pairs_ema_pk_rebuild m
        JOIN    analysis_forecasts.forecast_identities i
          ON    i.forecast_id = m.forecast_id;
    END IF;
    DROP TABLE analysis_forecasts.mov_pairs_ema_pk_rebuild;
    ALTER TABLE analysis_forecasts.mov_pairs_ema_new RENAME TO mov_pairs_ema;
    FOR r IN 0..15 LOOP
        EXECUTE format(
            'ALTER TABLE analysis_forecasts.mov_pairs_ema_new_p%s '
            'RENAME TO mov_pairs_ema_p%s',
