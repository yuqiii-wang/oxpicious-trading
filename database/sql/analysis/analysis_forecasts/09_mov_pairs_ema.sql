...
--  Comments
-- ----------------------------------------------------------------------------
COMMENT ON TABLE analysis_forecasts.mov_pairs_ema IS 'EMA-pair cross (golden/death cross) bucket definitions (motivation) — the EMA sibling of mov_pairs: one row per forecast_id — the window days of one security snapshot within the trailing 10-year window ending at the bucket''s stat_date where the stored relative-EMA spread of analysis.mov_ave_spreads_detail_ema changes sign: fast_leg=''ema6'' reads ema6_vs_ema{pair_window} = (ema6 - ema_{W}) / ema_{W}, fast_leg=''price'' reads price_vs_ema{pair_window} = (price - ema_{W}) / ema_{W} (the close-price leg, merged from the former separate mov_pairs_price_ema family). side=top a CROSS UP / golden cross (spread turns > 0 from <= 0, the fast leg rises through the slow EMA), side=bottom a CROSS DOWN / death cross (spread turns < 0 from >= 0), one-day signals (2026-09: the legacy fixed-5-day cooldown was removed; a cross day''s predecessor sits on the other side of zero so consecutive cross days are mutually exclusive — every cross day is its own forecast signal with streak_signal_days = 1). Keyed by the surrogate forecast_id (hash partition key); the shared identity (sec_type, code, stat_date) + bucket family live in analysis_forecasts.forecast_identities. Results (forward changes / reversal probabilities) live in analysis_forecasts.forecast_results via forecast_id. NO new EMA computation — the buckets read the parent mov_ave_spread analysis''s existing EMA spread columns. Source: analysis.mov_ave_spreads_detail_ema (ema6_vs_ema{W} / price_vs_ema{W}, W ∈ 60/120/255).';
...
COMMENT ON COLUMN analysis_forecasts.mov_pairs_ema.lookback_period IS 'Recorded build parameter (NOT a PK member): the trailing calendar window the bucket was computed over — ''10y'' = (stat_date - 10 years, stat_date]. Default ''10y''; a rebuild with a different lookback requires --force.';
