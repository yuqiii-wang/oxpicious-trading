...
--  CROSS (golden / death cross) event buckets built on the EXISTING
--  relative-spread columns of analysis.mov_ave_spreads_detail — the
--  parent mov_ave_spread analysis's own spread definitions. NO new MA
--  computation: a cross day is read directly off the stored spread's
--  sign flip. TWO fast legs per slow window (the fast_leg column):
--
--    fast_leg='ma5'   — ma5_vs_ma{W}   = (ma5   - ma_{W}) / ma_{W}
--    fast_leg='price' — price_vs_ma{W} = (price - ma_{W}) / ma_{W}
--
--  (the close-price leg merges the former separate mov_pairs_price
...
--  (the slow leg; the 5/20 windows exist in the source but are not
--  built).
--
...
CREATE TABLE IF NOT EXISTS analysis_forecasts.mov_pairs (
    ...
    CONSTRAINT ck_mov_pairs_regime
...
SELECT public.ensure_check_constraint(
    'analysis_forecasts.mov_pairs',
    'chk_mov_pairs_fast_leg',
    $chk$fast_leg IN ('ma5', 'price')$chk$);
