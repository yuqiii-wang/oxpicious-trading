python download_szse_trend.py
python download_sse_price.py
python download_szse_margin.py
python download_sse_margin.py
python download_csindex.py
python download_shibor.py
python download_chinabond.py
python download_pboc_repo_news.py
python download_sse_options_price.py

# build combined CSVs
python build_szse_sse_bse_stocks.py
python build_szse_sse_etf_and_margin.py
python build_csindex.py
python build_debt_baseline.py
python build_etf_classification.py
python build_index_classification.py
python build_szse_options.py
python build_stock_industry.py


# on monthly start date
# python download_sse_etf_composition.py
# python download_szse_etf_composition.py
# python download_index_composition.py
