...
export function IndustryEtfPriceChart({
  ...
  return (
    <BaseChart
      ...
      subtitle={subtitle}
      option={data && data.etfs.length > 0 ? option : null}
      loading={loading}
      error={error ? `Failed to load ETF price data: ${error}` : null}
      emptyText={emptyText}
      height={400}
      onCanvasClick={handleCanvasClick}
    >
      {/* In-plot controls (always visible — the base renders them in every
          state) */}
      <Stack spacing={1}>
