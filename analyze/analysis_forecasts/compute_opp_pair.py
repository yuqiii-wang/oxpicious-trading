    ...
    *,
    ...
    for mw in windows:
        ...
        for w in sorted(mats):
            ...
            # COLUMNAR motivation payload (the refactored
            # build_result_rows contract — a dict of (R,) lists aligned
            # with kk/ii, fanned out across the period rows there; never
            # a per-row dict loop).
            base: dict[str, list] = {
                "sec_type": [sec_type] * ii.size,
                "industry_id": [a_ids[i] for i in ii_l],
                "pair_industry_id": [b_ids[i] for i in ii_l],
                "stat_month": [mw.stat_month] * ii.size,
                "trend_window": [w] * ii.size,
                "side": [OPP_PAIR_SIDE] * ii.size,
                "benchmark_code": [benchmark_code] * ii.size,
                "pool_size": [pool_size] * ii.size,
