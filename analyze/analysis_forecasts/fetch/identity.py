...
async def fetch_forecast_identity(
    ...
    Returns the identity row (forecast_id, sec_type, code, stat_date,
    bucket, streak_signal_days, delayed_signal_days, lookback_period —
    code = the forecast subject: the security ticker) plus, when the
    bucket's motivation table is one of the known families, the full
    motivation row joined from it under the ``motivation`` key (dates
    arrive as ISO strings via the to_jsonb cast). None when the id is
    not registered.
