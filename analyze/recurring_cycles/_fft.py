"""FFT backend routing (cudf -> cupy -> cpu) shared by the recurring_cycles
...
def _rfft(windows: np.ndarray, axis: int = 1) -> np.ndarray:
    ...
    ``windows`` is a float64 ndarray. cuDF implements NO FFT, so the
    transform is routed EXPLICITLY to CuPy (cuFFT, GPU) when a CUDA
    device is present, else numpy (CPU). The returned complex128
    spectrum keeps all downstream logic unchanged.
    """
