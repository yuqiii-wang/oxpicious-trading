...
ANALYSIS_DESCRIPTION_OFFSETS = (
    ...
    "windows, RAW (overall_corr_ma{W}_{W}d) and after the common "
    "benchmark move is removed (offset_sub_corr_*): the offset curves are "
    "the CONSOLIDATED stats.cross_stats primitive "
    "code_price_with_benchmark_offset — per member index the daily "
    "close-change minus the benchmark's daily change — averaged over the "
    "industry's member indices per pool bucket (builds.industry "
    "classification: <51 small, <=180 mid, else large; compositionless "
    "members contribute to 'all' only) and cumulated from the window "
    "start (Pearson removes the per-window constant, so this equals the "
    "correlation of the offset levels). Derived "
    ...
    "stats.cross_stats.code_price_with_benchmark_offset (built by "
    "builds.cross_stats). Built by python -m analyze.analysis_composites "
    "(incremental / --force / --industry filtered)."
)
