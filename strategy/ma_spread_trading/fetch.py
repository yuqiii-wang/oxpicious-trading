"""DB fetch for the MA-spread strategy.

Strategy-specific: builds the SQL JOIN of
  - analysis.mov_ave_spreads_detail (MA-gap / slope / σ / turnover-MA columns)
  - analysis.mov_ave_rsi            (Wilder RSI + short-term gaps)
  - stats.<sec_type>_basic_stats    (open / close for fill prices)
...
from strategy.ma_spread_trading.config import (
    SIGNAL_COLUMNS,
...
def _build_fetch_sql(sec_type: str) -> str:
    ...
    table (b) for open / close. The LEFT JOIN on basic_stats means a row
    with a missing open price still survives (fill will be skipped in the
    backtest if open is NULL on the fill date).
...
async def fetch_signal_data(conn, sec_type: str, codes: list) -> pd.DataFrame:
    ...
    numeric_cols = list(SIGNAL_COLUMNS) + [
        ...
        "gap_since_last_extreme", "open_price", "close_price",
    ]
