...
from builds.industry.compute import (
    rebase_ohlc,
    aggregate_by_pool,
)
...
class IndustryBuild(DataBuild):
    """``python -m builds.industry`` — composite industry OHLC build."""

    title = ("BUILD INDUSTRY BASIC STATS "
    ...
    def header_fields(self) -> dict:
        return {
            ...
            "mode": "FORCE (full recompute)" if self.args.force
                    else f"DATE MODE (forced single date: {self.forced_date})"
