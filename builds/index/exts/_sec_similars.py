...
async def _build_for_sec_type(conn, sec_type: str, force: bool) -> None:
    ...
    full recompute when force=True (truncate sec_type rows first).
    """
    ...
    if force:
        ...
        target_dates: Optional[Set[datetime.date]] = source_dates
    else:
...
async def build_sec_similars(conn, force: bool = False) -> None:
    ...
    Incremental when force=False (missing composition snapshot dates only);
    full recompute when force=True (truncate per sec_type first).
    ...
    for sec_type in SEC_TYPES:
        await _build_for_sec_type(conn, sec_type, force)
