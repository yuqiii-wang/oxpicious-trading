...
stats.sec_index_tags, delegating each leaf's rows to its own upsert module in
hierarchy order:
...
NOTE: stats.sec_owners is no longer upserted here — it has been MOVED to
builds.sec_info (``python -m builds.sec_info``).  The in-memory owner
matchers (load_owners + build_owner_matchers + match_etf_owner) are still
used above this layer to populate the logical sec_classification.owner_id
reference; no DB sec_owners table is needed at classification time.
...
upserted in place (stale rows preserved).  sec_index_tags is always
truncated + rebuilt (needed for correctness when JSON is hand-edited).
Stock rows are always DELETEd + re-inserted (the set of qualifying indices
...
async def upsert_to_db(
    ...
    # NOTE: stats.sec_owners is now populated by builds.sec_info (run separately).
    # The logical sec_classification.owner_id reference is matched in-memory
    # above this layer (build_owner_matchers + match_etf_owner), so no DB
    # sec_owners row needs to exist at classification time.
    ...
    # --- 4. Recompute is_primary_exchange from exchange (derived column) ---
    await _update_is_primary_exchange(conn, verbose=verbose)

    # --- 5. Recompute is_active from identity tables (trailing 1 year) ---
...
async def _update_is_primary_exchange(conn, verbose: bool = True) -> None:
    """Recompute stats.sec_classification.is_primary_exchange from exchange.

    Derived column — ALWAYS generated by this rule (never set manually):
      SS / STAR / SZ / GEM / BJ  -> TRUE   (Greater-China primary exchanges)
      HK / OVERSEAS              -> FALSE  (cross-border / non-Greater-China)
      NULL / unrecognized        -> NULL   (synthetic DUMMY indices)

    Mirrors the ``_is_primary_exchange`` helper in
    sector_industry/exchange.py (the Python-side source of truth).  Runs as
    a post-upsert single UPDATE so every row — including stock rows that
    bypassed ON CONFLICT via DELETE+INSERT and any pre-existing rows in
    non-force mode — is re-derived from its final ``exchange`` value.
    """
    await conn.execute("""
        UPDATE stats.sec_classification
           SET is_primary_exchange = CASE
                WHEN exchange IN ('SS', 'STAR', 'SZ', 'GEM', 'BJ') THEN TRUE
                WHEN exchange IN ('HK', 'OVERSEAS') THEN FALSE
                ELSE NULL
           END
    """)
    if verbose:
        row = await conn.fetchrow(
            "SELECT COUNT(*) FILTER (WHERE is_primary_exchange) AS primary_ex, "
