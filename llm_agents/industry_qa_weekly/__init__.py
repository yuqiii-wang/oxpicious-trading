"""llm_agents.industry_qa_weekly — industry hypes & drains Q&A history.

Pick the industries that moved the market most over the last 5 trading
days — the top-5 HYPE (rose a lot vs the benchmark) and top-5 DRAIN
(dropped a lot) from analysis.industry_hypes_seasonal at the Market
Trend view's default period (120 trading days ≈ half a year) — the
exact monthly block the "Market Trend → Hypes & Drains" view renders —
and for each one ask the online-search summarize agent WHY (the
manually-asked convention in text.llm_qa, e.g.
...
(rise then drop, or drop then rise) is always asked. Current-month
anchors use a recent-movements question (drivers + fund flows).

Two AI-QA flows share the one knowledge base (text.llm_qa) — this
package is the HISTORY flow: the plan-first backfill replays the
episode-driven asks on the trading calendar (seasonal top + MA5-slope
deviation signals; categories ``industry`` / ``market``). The RECENCY
flow — the only LIVE generator since the live weekly step was removed —
is ``downloads.macro.ai_daily``: today / latest-biz-date broad-market
...
Package layout (small files, llm_agents-style):
  ...
  * ``questions`` — the question template (base + （截至date） suffix),
    ...
    check against text.llm_qa.
  * ``agent``     — run_backfill (the weekly step replayed on the
    5-trading-day grid — every step's window disjoint from the previous
    — oldest first, so dedupe behaves like real weekly runs) over the
    shared ask_one request -> store step.
  * ``cli``       — argparse CLI (backfill, --dry-run, --limit).
...
from llm_agents.industry_qa_weekly.core.agent import (
    ...
    PlanTooLargeError, run_backfill,
)
...
__all__ = [
    ...
    "CATEGORY", "AskOutcome", "PlanTooLargeError",
    "run_backfill", "main",
]
