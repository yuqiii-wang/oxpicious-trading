...
duplicated across stream_szse_price.py and stream_sse_price.py:
  ...
  * load_etf_member_codes(conn) — set of full codes ("600000.SS") in the
    ...
    is_in_etf on new stock_identity rows.
  * load_target_stocks(conn, lookback_days) — list of (bare_code, name) for
    ...
    is_in_etf=TRUE. Used by stream_szse_price.py as its streaming target list.
...
from _download_commons import setup_logger
...
# Lookback window (calendar days) for load_target_stocks. is_in_etf is
...
def load_etf_member_codes(conn) -> set:
    ...
    is_in_etf on new stock_identity rows so they don't default to FALSE
...
def load_target_stocks(
    ...
    Uses the pre-computed ``is_in_etf`` column on stock_identity (populated
